...
import type { sanctumLights, sanctumTextures } from "../data/sanctum.data";
import Gallery, {
	type GalleryModel,
...
export default class Sanctum extends Gallery {
	...
	initReflection() {
		...
		(sphereRenderTarget as any)._pmremGen = new THREE.PMREMGenerator(
			this._renderer,
		);
	...
	ufanReflection() {
		if (this.steel && this.steelCamera) {
			const renderTarget = (
				this.steelCamera!.renderTarget as any
			)._pmremGen.fromCubemap(this.steelCamera!.renderTarget.texture);
			if (this.steel) {
