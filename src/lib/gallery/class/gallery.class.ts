...
export interface GalleryModelUserdata {
	type: string;
	info: GalleryModelInfo;
}

export interface GalleryModel {
	id: number;
	userdata: GalleryModelUserdata;
...
/** 페인팅 배치용 아이템 (poster, 크기, 위치, 벽면) */
export interface GalleryPaintingItem {
	poster: string;
	w: number;
	h: number;
	position: THREE.Vector3;
...
export default class Gallery {
	...
	title: string;
	running: boolean;
	controls: GalleryControls | null;
	player: GalleryPlayer;
	isDragging: boolean;
	preventKeyFlag: boolean;
	paintingsArr: GalleryPaintingItem[];
	textures: typeof galleryTextures;
	models: GalleryModel[];
	lights: typeof galleryLights;
	pedeObject: Record<string, THREE.Mesh>;
	lightObject: THREE.SpotLight[];
	scaleFactorArr: number[];
	mergeArr: THREE.Vector3[];
	infoArr: GalleryModelUserdata[];
	loadingCount: number;
	rafId: string;
	idleTime: number;
	...
	paintingObject: Record<string, THREE.Mesh>;
	...
	addWallFloorCeiling() {
		...
		this.loadTexture(wTexture!)
			.then((texture: unknown) => {
				...
				let frontWall: THREE.Mesh;
				let leftWall: THREE.Mesh;
				let rightWall: THREE.Mesh;
				let backWall: THREE.Mesh;
				let middleWall: THREE.Mesh;
				// Front Wall
				if (this.title === "정방형공간") {
		...
		this.loadTexture(cTexture!)
			.then((texture: unknown) => {
				...
				let ceilingGeometry: THREE.PlaneGeometry;
	...
	addPaintings() {
		if (this.paintingsArr.length > 0) {
			this.paintingsArr.forEach((e, i) => {
				const painting = this.loadPainting(
					e.poster,
					e.w,
					e.h,
					e.position,
				...
				if (painting) this.paintingObject[`painting${i}`] = painting;
			});
			// scene add paintings
	...
	modelLoad(model: GalleryModel, position: THREE.Vector3) {
		return new Promise((resolve, reject) => {
			...
			this._modelLoader.load(
				...
				(gltf) => {
					...
					(gltf.scene as SceneObjectWithMeta).number = model.id;
					(gltf.scene as SceneObjectWithMeta).scaleFactor = scaleFactor;
					(gltf.scene as SceneObjectWithMeta).customProperty = "obj";
					gltf.scene.position.set(position.x, position.y, position.z);
					if (!this.objGroup) return;
					(
						this.objGroup as THREE.Group & { customProperty?: string }
					).customProperty = "objsGroup";
					if (!this._scene) return;
					this._scene.add(this.objGroup);

					resolve(gltf.scene as SceneObjectWithMeta);
				},
				(xhr) => {
	...
	createSpotlight(
		...
		// 벽처럼 빛을 비스듬히 받는 면의 shadow acne(가로줄) 감소
		...
		if (this.models.length > 0) {
			this.models.forEach((e, i) => {
				this.modelLoad(e, e.position)
					...
					.then((sceneObject: SceneObjectWithMeta) => {
	...
	loadPedestal(position: THREE.Vector3, scaleFactor: number, pHeight: number) {
		...
		(boxMesh as THREE.Mesh & { customProperty?: string }).customProperty =
			"pedestal";
	...
	addPedestal() {
		...
		this.models.forEach((e, i) => {
			if (e.pedestal) {
				// 좌대 있으면
				this.pedeObject[`pede${i}`] = this.loadPedestal(
					e.position,
					this.scaleFactorArr[i],
					4,
				);
			}
		});
	...
	checkCollision() {
		...
		for (let i = 0; i < this.wallGroup.children.length; i++) {
			const wall = this.wallGroup.children[i] as Object3DWithBoundingBox;
			if (
				wall.BoundingBox &&
				playerBoundingBox.intersectsBox(wall.BoundingBox)
			) {
				return true;
			}
		}
		...
		for (let i = 0; i < this.objGroup.children.length; i++) {
			const obj = this.objGroup.children[i] as Object3DWithBoundingBox;
			if (obj.BoundingBox && playerBoundingBox.intersectsBox(obj.BoundingBox)) {
				return true;
			}
		}
		if (!this.pedeGroup) return;
		for (let i = 0; i < this.pedeGroup.children.length; i++) {
			const pede = this.pedeGroup.children[i] as Object3DWithBoundingBox;
			if (
				pede.BoundingBox &&
				playerBoundingBox.intersectsBox(pede.BoundingBox)
			) {
				return true;
			}
		}
	...
	loopForCollision() {
		...
		for (let i = 0; i < this.wallGroup.children.length; i++) {
			const wall = this.wallGroup.children[i] as Object3DWithBoundingBox;
			wall.BoundingBox = new THREE.Box3();
			wall.BoundingBox.setFromObject(wall);
		}
		if (!this.objGroup) return;
		for (let i = 0; i < this.objGroup.children.length; i++) {
			const obj = this.objGroup.children[i] as Object3DWithBoundingBox;
		...
		for (let i = 0; i < this.pedeGroup.children.length; i++) {
			const pede = this.pedeGroup.children[i] as Object3DWithBoundingBox;
			pede.BoundingBox = new THREE.Box3();
			pede.BoundingBox.setFromObject(pede);
		}
	...
	controlSet() {
		...
		if (this.controls.w || this.controls.ArrowUp || this.controls.ㅈ) {
			...
			this._camera.position.add(
				direction.normalize().multiplyScalar(this.player.speed),
			);
		}
		...
		if (this.controls.a || this.controls.ArrowLeft || this.controls.ㅁ) {
			// a - left
			this._camera.position.add(
				right.normalize().multiplyScalar(-this.player.speed),
			);
		}
	...
	modelDispose(scene: THREE.Scene) {
		scene.children.forEach((e: THREE.Object3D) => {
			const obj = e as SceneObjectWithMeta;
			if (typeof obj.number === "number") {
				scene.remove(e);
				const first = obj.children[0] as THREE.Mesh | undefined;
	...
	rotate() {
		...
		this.canvasdiv.addEventListener("mousemove", (e) => {
			if (this.isDragging && this.controls) this.controls.onMouseMove(e);
		});
		...
		this.canvasdiv.addEventListener("touchstart", (e) => {
			this.isDragging = true;
			if (this.controls) this.controls.onTouchMove(e);
		});
		this.canvasdiv.addEventListener("touchend", () => {
			if (this.isDragging) {
				this.isDragging = false;
	...
	onMoveKeyDown(dataset: string | undefined) {
		if (!this.controls || dataset == null) return;
		this.controls[dataset] = true;
	}
	onMoveKeyUp(dataset: string | undefined) {
		if (!this.controls || dataset == null) return;
