...
export default class SmallGallery extends Gallery {
	...
	initCamera() {
		...
		if (this.title === "small-gallery") {
			// 공간에 따라 시작위치 변경
			this._camera.position.set(0, this.player.height, 7);
	...
	addWallFloorCeiling() {
			...
			.then((tex: unknown) => {
				...
				if (this.title === "small-gallery") {
					frontWall = new THREE.Mesh(
						new THREE.BoxGeometry(30, 30, 0.001), // (가로, 높이, 두께)
						new THREE.MeshLambertMaterial({ map: wallTexture }),
					);
					frontWall.position.z = -15; // push the wall forward in the Z axis
		...
		this.loadTexture(cTexture!) // load the image/texture
			.then((tex: unknown) => {
				...
				if (this.title === "small-gallery") {
					ceilingGeometry = new THREE.PlaneGeometry(30, 30);
				}
		...
		return new Promise((resolve, reject) => {
			this._modelLoader!.load(
				...
				(gltf) => {
					...
					if (gltf.scene.name === "The Dawn of Christianity") {
	...
	objectAnimation() {
		if (this.objGroup?.getObjectByName("The Dawn of Christianity")) {
			const to = this.objGroup.getObjectByName("The Dawn of Christianity");
			if (to) {
