import { type Camera, Euler, EventDispatcher, Vector3 } from "three";
...
class PointerLockControls extends EventDispatcher<ControlsEvent> {
	...
	onTouchMove(e: TouchEvent) {
		...
		switch (e.touches.length) {
			case 1:
				...
				if (e.touches[0].target === this.domElement.querySelector("canvas"))
					touch = e.touches[0];
				break;
			case 2:
				if (e.touches[0].target === this.domElement.querySelector("canvas"))
					...
					e.touches[1].target === this.domElement.querySelector("canvas")
				)
					touch = e.touches[1];
				break;
		}
	...
	onPointerlockChange() {
		if (
			...
			this.domElement.querySelector("canvas")
		) {
	...
	connect() {
		this.domElement.addEventListener(
			"touchmove",
			this.onTouchMoveBind as EventListener,
		);
		this.domElement.addEventListener(
			"touchend",
			this.onTouchEndBind as EventListener,
		);
	...
	unlock() {
		if (
			typeof this.domElement.querySelector("canvas")?.requestPointerLock !==
			"undefined"
		)
