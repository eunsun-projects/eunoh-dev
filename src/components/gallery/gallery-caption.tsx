...
function GalleryCaption({ isInfoOpen }: GalleryCaptionProps) {
	...
	return (
		<div style={{ display: isInfoOpen.open === true ? "block" : "none" }}>
			...
			>
				<h3 className="relative mb-3 inline-block font-normal text-2xl text-white">
				...
				<MdOpenInFull
					...
					onClick={handleBig}
				/>
				<p className="word-break-keep-all m-0 text-lg text-white">
					{`artist : ${isInfoOpen.info.artist}`}
				</p>
				<p className="word-break-keep-all m-0 text-lg text-white">{`year : ${isInfoOpen.info.year}`}</p>
				<p className="word-break-keep-all m-0 text-lg text-white">{`material : ${isInfoOpen.info.material}`}</p>
			</div>

			{/* 여기를 보이게하고*/}
			<div
				className="fixed top-1/2 left-1/2 w-1/2 min-w-48 translate-x-1/2 translate-y-1/2 transform rounded-md border-2 border-white bg-slate-300/40 p-2.5 text-white backdrop-blur-sm"
