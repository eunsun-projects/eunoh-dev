...
function Model() {
	...
	useFrame(({ clock }, delta) => {
		...
		if (play) {
			if (refs.current.lamp) {
				refs.current.lamp.position.z += delta * 24;
			}
			if (refs.current.lamp2) {
	...
	useEffect(() => {
		for (let i = 0; i < loadedArr.length; i++) {
			for (const e of loadedArr[i].scenes) {
				setSceneName(e, modelData[i].name);
			}
		}
	}, [loadedArr]);
			...
			{objects === 1 && (
				<primitive
					...
					ref={(element: Object3D<Object3DEventMap>) =>
						// biome-ignore lint/suspicious/noAssignInExpressions: for element
