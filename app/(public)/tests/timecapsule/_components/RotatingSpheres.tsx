...
function RotatingSpheres() {
	...
	const handleClick = useCallback(
		(timeCapsule: TimeCapsuleFromSupabase) => (_e: ThreeEvent<MouseEvent>) => {
			const matchedTimeCapsule = timeCapsules.find(
				(item) => item.id === timeCapsule.id,
			);
			setFocusedObject({
				isIdle: false,
				timeCapsule: matchedTimeCapsule || null,
			});
		},
	...
	useFrame((state, delta) => {
		...
		if (focusedObject?.isIdle) {
			...
			(state.controls as OrbitControls).target.lerp(
