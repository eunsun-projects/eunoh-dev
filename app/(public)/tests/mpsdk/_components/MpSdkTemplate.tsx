  ...
  useEffect(() => {
    const configure = async (mpSdk: MpSdk) => {
      ...
      if ('obj3D' in lightNode) {
        ...
        (obj3D.children[1].children[0] as THREE.PointLight).shadow.mapSize.width = 1024;
        (obj3D.children[1].children[0] as THREE.PointLight).shadow.mapSize.height = 1024;
        (obj3D.children[1].children[0] as THREE.PointLight).shadow.camera.near = 1;
        (obj3D.children[1].children[0] as THREE.PointLight).shadow.camera.far = 10000;
        (obj3D.children[1].children[0] as THREE.PointLight).shadow.camera.focus = 1;
        (obj3D.children[1].children[0] as THREE.PointLight).shadow.needsUpdate = true;
      }
    };
    if (mpsdk) {
      configure(mpsdk);
